...
export const handler = async (event) => {
  ...
  try {
    const existing = await sql`SELECT id, vote_type FROM votes WHERE user_id = ${user.id} AND target_id = ${targetId} AND target_type = ${targetType}`
    ...
    if (existing.length) {
      ...
      await sql`INSERT INTO votes (user_id, target_id, target_type, vote_type) VALUES (${user.id}, ${targetId}, ${targetType}, ${voteType})`
      if (targetType === 'post') {
        if (voteType === 'up') await sql`UPDATE posts SET upvotes = upvotes + 1 WHERE id = ${targetId}`
        else await sql`UPDATE posts SET downvotes = downvotes + 1 WHERE id = ${targetId}`
      } else {
    ...
    return error('Failed to vote: ' + e.message, 500)
  }
}
