...
export const handler = async (event) => {
  ...
  if (event.httpMethod === 'GET' && !sub) {
    try {
      ...
      const posts = await sql`
        ...
        FROM posts p LEFT JOIN users u ON p.author_id = u.id WHERE p.id = ${postId}`
      ...
      const comments = await sql`
        SELECT c.*, u.username as author_username, u.role as author_role, u.is_verified as author_is_verified,
          (SELECT vote_type FROM votes WHERE user_id = ${userId} AND target_id = c.id AND target_type = 'comment') as user_vote
      ...
      return json({
        ...
        comments: comments.map(c => ({
          ...
          isAnonymous: c.is_anonymous, upvotes: c.upvotes, userVote: c.user_vote, createdAt: c.created_at
        }))
      })
    } catch (e) { return error('Failed to fetch post: ' + e.message, 500) }
  }
  ...
  if (event.httpMethod === 'DELETE' && !sub) {
    ...
    try {
      ...
      return json({ ok: true })
    } catch (e) { return error('Failed to delete: ' + e.message, 500) }
  }
  ...
  if (event.httpMethod === 'POST' && sub === 'comments') {
    ...
    try {
      ...
      return json({
        ...comments[0], postId: comments[0].post_id, parentId: comments[0].parent_id,
        isAnonymous: comments[0].is_anonymous, createdAt: comments[0].created_at,
        authorId: user.id, authorUsername: isAnonymous ? null : user.username,
        authorRole: user.role, authorIsVerified: user.is_verified, upvotes: 0, userVote: null
      })
    } catch (e) { return error('Failed to post comment: ' + e.message, 500) }
  }
